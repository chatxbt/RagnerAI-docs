...
Upload any protocol documentation (APIs, RPCs, GraphQL, Smart Contracts), and Ragner instantly generates:
...
// interacting with a ragner salesforce agent
"Show me real-time sales data from Salesforce API" -> "GET /sales/realtime

// user asks  -> agent makes api call -> returns data
```

```python SDK Integration Example:
#   Turn CoinGecko's API documentation into an Agent
from ragner import rAgent

defi_agent = rAgent.from_docurl("https://docs.coingecko.com/v3.0.1/")
...
Like building with LEGO blocks, leverage composable swarms to create multi-step workflows by combining multiple Ragnar API Agents without writing code.
